...
- 软件环境配置
  - Nginx
    ...
    - 停止运行Nginx，避免运行VeryNginx时出现端口占用

      ```cmd
      idchannov@id-srv:~$ sudo systemctl stop nginx
      ```
  ...
  idchannov@id-srv:~$ /opt/verynginx/openresty/nginx/sbin/nginx
    ...
    - 停止服务
      ...
      ```cmd
      idchannov@id-srv:~$ sudo /opt/verynginx/openresty/nginx/sbin/nginx -s stop
      ```
  ...
  - Damn Vulnerable Web Application
    ...
    - 为DVWA创建MySQL数据库和用户
      ...
      # 创建一个单独的MySQL用户帐户，该帐户将专用于wordpress数据库
      mysql> GRANT ALL ON wordpress.* TO 'dvwauser'@'localhost' IDENTIFIED BY 'password';
      # 重新加载授权表并使新的更改生效
      mysql> FLUSH PRIVILEGES;
      # 退出MySQL
      mysql> exit;
  ...
  server {
    ...
    ```cmd
      idchannov@id-srv:~$ vim /var/www/html/config/config.inc.php
    idchannov@id-srv:~$ vim /etc/php/7.2/apache2/php.ini
      # 更改文件夹的属主信息
...
- **基本要求**
  ...
  - 在一台主机（虚拟机）上同时配置Nginx和VeryNginx
    - VeryNginx作为本次实验的Web App的反向代理服务器和WAF
      ...
      （2）在Proxy Pass中添加配置信息
    
    - PHP-FPM进程的反向代理配置在nginx服务器上，VeryNginx服务器不直接配置Web站点服务
    
      ```cmd
      # 修改配置文件
      idchannov@id-srv:~$ sudo vim /etc/nginx/sites-available/default
      # 重启Nginx
  ...
  - 使用Wordpress搭建的站点对外提供访问的地址为：http://wp.sec.cuc.edu.cn
    ...
    - 修改主机的hosts文件，增加`idchannovのip wp.sec.cuc.edu.cn`
    - 在`/etc/nginx/sites-available/`目录中建立一个新的服务器块配置文件，名称为`wp.sec.cuc.edu.cn`，并进行[配置文件](config.txt)的编辑
  
  - 使用Damn Vulnerable Web Application (DVWA)搭建的站点对外提供访问的地址为： http://dvwa.sec.cuc.edu.cn
  
    - 修改主机的hosts文件，增加`idchannovのip dvwa.sec.cuc.edu.cn`
...
- 按操作说明配置完WordPress和DVWA之后无法打开默认配置页面
  ...
  **解决办法：**
  ...
  - 修改WordPress和DVWA的配置文件，在index里添加index.php
